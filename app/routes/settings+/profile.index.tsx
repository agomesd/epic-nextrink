...
export async function action({ request }: DataFunctionArgs) {
	...
	const submission = await parse(formData, {
		...
		schema: profileFormSchema.superRefine(
			async ({ email, currentPassword, newPassword }, ctx) => {
				...
				if (currentPassword && newPassword) {
					const user = await verifyLogin(email, currentPassword)
					if (!user) {
						ctx.addIssue({
	...
	const { email, newPassword } = submission.value
	...
	return redirect(`/users/${updatedUser.id}`, { status: 302 })
}
